...
var FormCheck = new Class({
	...
	options : {
		...
		onAjaxRequest : Class.empty,				//Function to fire when the Request event starts 
		onAjaxSuccess : Class.empty,				//Function to fire when the Request receives .  Args: response [the request response] - see Mootools docs for Request.onSuccess 
		onAjaxFailure : Class.empty,				//Function to fire if the Request fails
		
		onSubmit		  : Class.empty,			//Function to fire when user submit the form
		onValidateSuccess : Class.empty,			//Function to fire when validation pass
		onValidateFailure : Class.empty,			//Function to fire when validation fails
	...
	initialize : function(form, options) {
		if (this.form = $(form)) {
			...
			this.form.getElements("*[class*=validate]").each(function(el) {
				if (el.getTag() == 'select' || el.getTag() == 'input' || el.getTag() == 'textarea') this.register(el);
			}, this);
	...
	*/
	validate : function(el) {
		...
		if (!this.options.validateDisabled && el.getProperty('disabled')) return true;
	...
	*/
	simpleValidate : function(el) {
		if (el.getTag() == 'select' && el.selectedIndex <= 0) {
	...
	*/
	validateDiffers: function(el,ruleArgs) {
		...
		if(el.value == this.form[differs].value){
			var msg;
			if (this.options.display.titlesInsteadNames)
				msg = this.options.alerts.differs.replace("%0",this.form[differs].getProperty('title'));
			else
				msg = this.options.alerts.differs.replace("%0",differs);
			el.errors.push(msg);
			return false;
		}
		return true;
	...
	*/
	addError : function(obj) {
		...
		if (obj.element && obj.element != true) {
			...
			if (!this.options.display.fadeDuration || this.options.display.errorsLocation < 2) {
	...
	*/
	removeError : function(obj, method) {
		...
		if (!this.options.display.fadeDuration || this.options.display.errorsLocation == 1 && obj.element) {
			this.fxRunning = true;
			obj.element.remove();
			obj.element = false;
	...
	submitByAjax: function() {
		...
		new Request({
			...
			onSuccess: function(result){
				...
				if(this.options.ajaxResponseDiv) $(this.options.ajaxResponseDiv).setHTML(result);
			}.bind(this)
		}).send();
	},
...
FormCheck.implement(new Events, new Options);